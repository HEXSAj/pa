...
const ExportSalesButton = ({ sales, isFiltered, allSales }: ExportSalesButtonProps) => {
  ...
  const prepareExcelData = (salesData: Sale[]) => {
    return salesData.map(sale => {
      // For free bills, selling price is 0 but we need to use original amount for profit margin calculation
      const sellingPrice = sale.totalAmount;
      const originalAmount = sale.originalAmount || sellingPrice;
      const profit = sellingPrice - sale.totalCost;
      const profitMargin = originalAmount > 0 ? ((profit / originalAmount) * 100).toFixed(1) : '0.0';
      ...
      return {
        ...
        'Sale Type': sale.isFreeBill ? 'Free Bill (100% Discount)' : 'Regular Sale',
        'Original Amount': sale.isFreeBill && sale.originalAmount ? `Rs${sale.originalAmount.toFixed(2)}` : '-',
        'Total Amount': `Rs${sellingPrice.toFixed(2)}`,
        ...
        'Items Details': sale.items.map(item => 
          `${item.item.name} (${item.unitQuantity} units${item.subUnitQuantity ? ` + ${item.subUnitQuantity} ${item.item.unitContains?.unit || ''}` : ''}) - Rs${sale.isFreeBill ? 0 : item.totalPrice.toFixed(2)}`
        ).join('\n'),
