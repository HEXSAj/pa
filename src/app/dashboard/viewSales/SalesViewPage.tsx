...
import SalesFilter, { DateFilterType } from './SalesFilter';
import { Badge } from "@/components/ui/badge";
import EnhancedSalesSummary from './EnhancedSalesSummary';
import { EnhancedSalesTable } from './EnhancedSalesTable';
import { EnhancedSaleDetails } from './EnhancedSaleDetails';
...
function SalesViewPage() {
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<Customer | undefined>();
  const [selectedStaff, setSelectedStaff] = useState<StaffUser | undefined>();
  // Removed selectedPatientType since all sales are processed uniformly
  const [dateFilter, setDateFilter] = useState<DateFilterType>('all');
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(undefined);
  const [selectedMonth, setSelectedMonth] = useState<string | undefined>(undefined);
  const [selectedYear, setSelectedYear] = useState<string | undefined>(undefined);
  ...
  // Removed activeTab since we no longer use tabs
  ...
  const loadSales = async () => {
    try {
      setLoading(true);
      const data = await saleService.getAll();
      // Filter to only show sales with inventory items
      const salesWithInventory = data.filter(sale => 
        sale.items && 
        sale.items.length > 0
      );
      setSales(salesWithInventory);
  ...
  const filterSales = (sales: Sale[]) => {
    ...
    if (selectedCustomer) {
      filteredSales = filteredSales.filter(sale => 
        sale.customer?.id === selectedCustomer.id ||
        (sale.customerInfo?.name === selectedCustomer.name && 
         sale.customerInfo?.mobile === selectedCustomer.mobile)
      );
    }
    ...
    // Note: All sales are processed the same way, so no patient type filtering needed
  ...
  // All sales are displayed the same way
      ...
      <div className="space-y-4 p-6">
        ...
        {activeView === 'dashboard' ? (
          ...
          <>
            ...
            {/* Sales Summary */}
            <EnhancedSalesSummary sales={filteredSales} />
        ...
        <Dialog 
          ...
          onOpenChange={setShowDetailsDialog}
        >
          <DialogContent className="max-w-6xl w-[95vw] h-[95vh] p-0 flex flex-col overflow-hidden">
            <DialogTitle className="sr-only">Sale Details</DialogTitle>
            {selectedSale && (
              <EnhancedSaleDetails 
                sale={selectedSale}
